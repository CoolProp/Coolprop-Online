...
def check_form(form_vars):
    ...
    try:
        ...
        key1 = CoolProp.CoolProp.get_parameter_index(input_longname_to_key[form_vars.get('name1', '')])
        key2 = CoolProp.CoolProp.get_parameter_index(input_longname_to_key[form_vars.get('name2', '')])
        # Try to get the pair
        CoolProp.CoolProp.generate_update_pair(key1, float(form_vars.get('value1', 0)),
                                               key2, float(form_vars.get('value2', 0)))
    ...
    form = FORM(
        TABLE(
            TR(
                TD("Fluid"),
                TD(SELECT(
                    *[OPTION(f, _value=f, _selected="selected" if f == "Ammonia" else None) for f in fluids],
                    _name="fluid"
                ))
            ),
            TR(
                TD("Input #1"),
                TD(SELECT(
                    *[OPTION(opt, _value=opt, _selected="selected" if opt == "Pressure [Pa]" else None) for opt in input_long_strings],
                    _name="name1"
                ))
            ),
            TR(
                TD("Value #1"),
                TD(INPUT(_type="text", _name="value1", _value="101325"))
            ),
            TR(
                TD("Input #2"),
                TD(SELECT(
                    *[OPTION(opt, _value=opt, _selected="selected" if opt == "Temperature [K]" else None) for opt in input_long_strings],
                    _name="name2"
                ))
            ),
            TR(
                TD("Value #2"),
                TD(INPUT(_type="text", _name="value2", _value="298"))
            ),
            TR(
                TD("Output Units"),
                TD(SELECT(
...
def next():
    ...
    if unit_system == 'Mole-based':
        entries += [
            ...
            TR(TD("Constant-pressure specific heat [J/mol/K]"), TD(f"{HEOS.cpmolar():.6g}")),
            TR(TD("Constant-volume specific heat [J/mol/K]"), TD(f"{HEOS.cvmolar():.6g}"))
        ]
        ...
        entries += [
            ...
            TR(TD("Constant-pressure specific heat [J/kg/K]"), TD(f"{HEOS.cpmass():.6g}")),
            TR(TD("Constant-volume specific heat [J/kg/K]"), TD(f"{HEOS.cvmolar():.6g}"))
        ]
    ...
    fig_html = XML(fig.to_html(include_plotlyjs='cdn', div_id='saturation-plot', config={'responsive': True}))

    # Get CoolProp version info
    cp_version = CoolProp.__version__
    cp_gitrev = CoolProp.__gitrevision__

    return dict(form=form, fig=fig_html, fluid=fluid, cp_version=cp_version, cp_gitrev=cp_gitrev)
